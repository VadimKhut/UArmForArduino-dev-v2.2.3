...
{
	// Projection length in the XY plane
	...
	// Projection length in the Z axis
...
unsigned char uArmController::xyzToAngle(double x, double y, double z, double& angleRot, double& angleLeft, double& angleRight, boolean allowApproximate)
{
	...
	else
	{
		if (x > 0)
		{
			angleRot = atan(y / x) * MATH_TRANS;	//angle tranfer 0-180 CCW,  угол в Радиан 
		}
		if (x < 0)
		{
	...
	sqrtZX = sqrt(zIn*zIn + xIn*xIn);    // Length of joint 2 to joint 4
